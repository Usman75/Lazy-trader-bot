...
// ChainLink Ropsten priceFeed Consumer Contract
const PriceConsumer = [
	{
		"inputs": [],
...
const PriceConsumer_Address = '0x156e02A050617F36D78cD09D86cdA0b9Ddb2D9b7'
const PriceConsumer_Contract = new web3.eth.Contract(PriceConsumer, PriceConsumer_Address);
...
async function monitorPrice() {
  ...
  try {
    ...
    let  usdAmount = await PriceConsumer_Contract.methods.getLatestPrice().call()
    let  timeStamp = await PriceConsumer_Contract.methods.getLatestPriceTimestamp().call()
    const CHECK_ETH_SELL_PRICE = web3.utils.fromWei(ETH_SELL_PRICE.toString(), 'Ether')
    //const atTime = new Date('timeStamp')
    const price = usdAmount / 100000000
    console.log('Eth Price:', price, ' USD AT ' ,new Date(timeStamp * 1000).toTimeString())
    // console.log('Selling Eth Price = ', ETH_SELL_PRICE )
    // console.log('Comp_ Price = ', comp_price)
    if(price <= CHECK_ETH_SELL_PRICE) {
